...
@Component
public class EventReliabilitySchemaInitializer implements ApplicationRunner {

    private static final Logger log = LoggerFactory.getLogger(EventReliabilitySchemaInitializer.class);

    private final JdbcTemplate jdbcTemplate;
    private final boolean enabled;

    public EventReliabilitySchemaInitializer(
            JdbcTemplate jdbcTemplate,
            @Value("${event.reliability.schema-auto-init:true}") boolean enabled
    ) {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            jdbcTemplate.execute(eventOutboxSql());
            jdbcTemplate.execute(eventConsumeLogSql());
            jdbcTemplate.execute(eventFailureRecordSql());
            jdbcTemplate.execute(searchSyncCheckpointSql());
        } catch (Exception e) {
            log.warn("Event reliability schema initialization failed", e);
        }
    }

    private String eventOutboxSql() {
        return """
    ...
    private String eventConsumeLogSql() {
        return """
    ...
    private String eventFailureRecordSql() {
        return """
    ...
    private String searchSyncCheckpointSql() {
        return """
