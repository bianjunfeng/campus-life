<template>
  <div class="admin-member-login-log">
    <!-- 筛选条件 -->
      ...
      <div class="dialog" @click.stop>
        ...
        <div class="detail-content" v-if="selectedLog">
          ...
          <div class="detail-row">
            ...
            <span class="detail-value">{{ selectedLog.memberId }}</span>
          </div>
          <div class="detail-row">
...
import { getMemberLoginLogList, type MemberLoginLog } from '../../api/admin'

const logList = ref<MemberLoginLog[]>([])
const page = ref(1)
const size = ref(10)
const total = ref(0)
...
const selectedLog = ref<MemberLoginLog | null>(null)
...
const loadLogs = async () => {
  try {
    ...
    if (filterMemberId.value) {
      params.memberId = parseInt(filterMemberId.value)
    }
    ...
    const data = await getMemberLoginLogList(params)
    logList.value = data.list
    total.value = data.total
  } catch (error) {
    console.error('加载会员登录日志失败:', error)
  }
}
...
const viewDetails = (log: MemberLoginLog) => {
